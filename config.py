...
from environment.setting import *


# 配置环境基类
class Config(object):
    """
    引用文档： http://docs.jinkan.org/docs/flask/config.html
    ######基础配置####
    DEBUG: 启用/禁用调试模式
    TESTING: 启用/禁用测试模式
    ...
    SQLALCHEMY_ECHO: 打印sql语句到控制台
    SQLALCHEMY_COMMIT_ON_TEARDOWN: 事务自动提交,可选配置 这样我们做了增删改操作之后就不用一次又一次commit了
    SQLALCHEMY_TRACK_MODIFICATIONS: 禁用修改跟踪
    """

    # 密钥设置，可自己设置值
    SECRET_KEY = flask_demo_secret_key

    # mysql数据库设置：https://zhuanlan.zhihu.com/p/65243645
    SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://%s:%s@%s:%s/%s' % (flask_demo_mysql_user, flask_demo_mysql_password,
                                                                  flask_demo_mysql_host, flask_demo_mysql_port,
                                                                  flask_demo_mysql_dbname)
    SQLALCHEMY_COMMIT_ON_TEARDOWN = True
    SQLALCHEMY_TRACK_MODIFICATIONS = False

    # session设置： https://www.cnblogs.com/cwp-bg/p/9339865.html
    SESSION_TYPE = 'redis'
    if flask_demo_redis_password:
        SESSION_REDIS = StrictRedis(host=flask_demo_redis_host, port=flask_demo_redis_port, db=flask_demo_redis_dbname,
                                    password=flask_demo_redis_password)
    else:
        SESSION_REDIS = StrictRedis(host=flask_demo_redis_host, port=flask_demo_redis_port, db=flask_demo_redis_dbname)
